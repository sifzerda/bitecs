...
import { releaseAsteroidEntity } from "../pools/asteroidPool.js"
...
export function killBoss(id, x, y) {
    ...
    // Unlock weapon
    if (
        weaponId != null && !simState.unlockedWeapons.includes(weaponId)
    ) {
        simState.unlockedWeapons.push(weaponId)
    }

    simState.pendingUnlockWeapon = weaponId
    useGameStore.getState().completeCurrentLevel()
