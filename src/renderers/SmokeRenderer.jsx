...
import { smokeSources } from "../effects/gpu/SmokeState"
     ...
     vec2 expand = right * engineSide * age * mix(0.9, 2.4, uBoost);
      ...
      if (life >= 0.0) {
        if (uEmitting > 0.5) {
          ...
          float speed = 2.0 + seed * 8.0;
          vec2 launchVel = vec2(cos(angle), sin(angle)) * speed;
          pos = uShipPos;
          life = 2.0 + seed * 2.0;
...
export function ExhaustRenderer({
  ...
  useFrame((state, delta) => {
   ...
   // console.log(exhaustSources)
    ...
    const source = smokeSources.find(s => s.slot === slot)
