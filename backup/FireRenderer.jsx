...
// this is for any fire fx — point-sprite based, each flame is a short
// wavy strand of stacked sprites (not a single billboard quad)
...
// segments per flame strand — tip tapers off the last one
const SEGMENTS = 7

const hot = new THREE.Color(3.2, 2.9, 1.6)   // overdriven for bloom
const mid = new THREE.Color(2.6, 1.1, 0.08)
const cool = new THREE.Color(0.9, 0.12, 0.02)
const smoke = new THREE.Color(0.12, 0.1, 0.1)
...
export function FireRenderer() {
    ...
    useFrame((state) => {
        ...
        for (let i = 0; i < capacity; i++) {
            ...
            const t = Math.max(0, Math.min(1, p.life[i] / p.maxLife[i])) // 1 -> 0
            const age = 1 - t
            ...
            // strand grows quickly on spawn then relaxes as the particle ages
            ...
            for (let s = 0; s < SEGMENTS; s++) {
                ...
                const idx = base + s
                const frac = s / (SEGMENTS - 1) // 0 at root, 1 at tip

                // layered sine waves = the "fluid waving" motion, more
                // pronounced toward the tip, phase-offset per particle
                ...
                tmpColor.copy(hot).lerp(mid, THREE.MathUtils.smoothstep(frac, 0.15, 0.7))
                tmpColor.lerp(cool, THREE.MathUtils.smoothstep(frac, 0.7, 1.0))
                tmpColor.lerp(smoke, THREE.MathUtils.smoothstep(age, 0.6, 1.0) * 0.6)
