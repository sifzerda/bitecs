...
export function BossRenderer() {
    ...
    const fuselage = useControls('Boss / Fuselage', {
        color: "#ddfdff",
        tipY: { value: 0.68, min: 0.2, max: 2, step: 0.01 },
        shoulderY: { value: 0.16, min: -1, max: 2, step: 0.01 },
        shoulderWidth: { value: -0.73, min: 0, max: 1, step: 0.01 },
        waistY: { value: -0.73, min: -1.5, max: 1.5, step: 0.01 },
        waistWidth: { value: 0.13, min: 0, max: 1, step: 0.01 },
        tailY: { value: -0.42, min: -2, max: 0, step: 0.01 },
        tailWidth: { value: 0.0, min: 0, max: 1, step: 0.01 },
        notchY: { value: -2.00, min: -2, max: 0, step: 0.01 },
    }, { collapsed: true })

    const cockpit = useControls('Boss / Cockpit', {
        color: '#ffbf00',
        topY: { value: 0.11, min: 0, max: 2, step: 0.01 },
        topWidth: { value:0.11, min: 0, max: 0.5, step: 0.01 },
        midY: { value: 0.39, min: -1, max: 2, step: 0.01 },
        midWidth: { value: 0.16, min: 0, max: 0.5, step: 0.01 },
        bottomY: { value: 0.46, min: -1, max: 2, step: 0.01 },
        bottomWidth: { value: 0.0, min: 0, max: 0.5, step: 0.01 },
    }, { collapsed: true })

    const wing = useControls('Boss / Wing', {
        color: '#ff2d2d',
        rootX: { value: 0.0, min: 0, max: 1, step: 0.01 },
        rootY: { value: 0.57, min: -1, max: 1, step: 0.01 },
        tipX: { value: 0.0, min: 0, max: 2, step: 0.01 },
        tipY: { value: -1.50, min: -1.5, max: 1.5, step: 0.01 },
        trailX: { value: 0.67, min: 0, max: 2, step: 0.01 },
        trailY: { value: 0.63, min: -1.5, max: 1.5, step: 0.01 },
        innerX: { value: 0.79, min: 0, max: 1, step: 0.01 },
        innerY: { value: 0.04, min: -1.5, max: 1.5, step: 0.01 },
    }, { collapsed: true })
    ...
    const wingtip = useControls('Boss / Wingtip', {
        color: '#003479',
        width: { value: 0.11, min: 0, max: 0.3, step: 0.005 },
        height: { value: 0.66, min: 0, max: 1.5, step: 0.01 },
        offsetX: { value: 0.43, min: 0, max: 2, step: 0.01 },
        offsetY: { value: -0.04, min: -1.5, max: 1.5, step: 0.01 },
    ...
    const cockpitGlass = useControls('Boss / Cockpit Glass', {
        ...
        roughness: { value: 0.15, min: 0, max: 1, step: 0.005 },
        transmission: { value: 1, min: 0, max: 1, step: 0.01 },
        thickness: { value: 0.72, min: 0, max: 3, step: 0.01 },
        ior: { value: 1.00, min: 1, max: 2.5, step: 0.01 },
    ...
    const engineIntake = useControls('Boss / Engine Intake', {
        enabled: true,
        color: '#00439a',
        width: { value: 0.11, min: 0, max: 0.5, step: 0.01 },
        height: { value: 0.63, min: 0, max: 1, step: 0.01 },
        offsetX: { value: 0.18, min: 0, max: 1.5, step: 0.01 },
        offsetY: { value: 0.39, min: -1.5, max: 1.5, step: 0.01 },
    }, { collapsed: true })
    ...
    const hullVent = useControls('Boss / Hull Vent', {
        ...
        width: { value: 0.28, min: 0, max: 1.0, step: 0.01 },
        height: { value: 0.29, min: 0, max: 0.3, step: 0.005 },
        spacing: { value: 0.10, min: 0.01, max: 0.3, step: 0.005 },
        offsetX: { value: 0.35, min: 0, max: 1, step: 0.01 },
        offsetY: { value: 0.24, min: -1, max: 1, step: 0.01 },
    }, { collapsed: true })
    ...
    const noseSpike = useControls('Boss / Nose Spike', {
        enabled: true,
        color: '#ff2d2d',
        length: { value: 0.37, min: 0, max: 1, step: 0.01 },
        width: { value: 0.16, min: 0, max: 0.5, step: 0.01 },
        offsetY: { value: -0.32, min: -0.5, max: 0.2, step: 0.01 },
        roundness: { value: 0.94, min: 0, max: 1, step: 0.01 },
        zOffset: { value: 0.04, min: 0, max: 0.1, step: 0.001 },
    }, { collapsed: true })
    ...
    const tailFin = useControls('Boss / Tail Fin', {
        ...
        width: { value: 0.38, min: 0, max: 1, step: 0.01 },
        sweep: { value: 0.00, min: 0, max: 1, step: 0.01 },
        offsetX: { value: 0.24, min: 0, max: 1, step: 0.01 },
        offsetY: { value: -0.22, min: -1.5, max: 1.5, step: 0.01 },
    ...
    const exhaustPort = useControls('Boss / Exhaust Port', {
        enabled: true,
        color: '#ff2d2d',
        width: { value: 0.17, min: 0, max: 1, step: 0.01 },
        height: { value: 0.46, min: 0, max: 2.5, step: 0.01 },
        offsetX: { value: 0.0, min: -0.5, max: 0.5, step: 0.01 },
    ...
    const tailBoom = useControls('Boss / Tail Boom', {
        enabled: true,
        color: '#3a6bd5',
        length: { value: 0.25, min: 0, max: 2, step: 0.01 },
        baseWidth: { value: 0.17, min: 0, max: 0.6, step: 0.01 },
        tipWidth: { value: 0.06, min: 0, max: 0.4, step: 0.01 },
    }, { collapsed: true })
    ...
    const boomFin = useControls('Boss / Boom Fins', {
        ...
        length: { value: 1.00, min: 0, max: 1, step: 0.01 },
        width: { value: 0.17, min: 0, max: 1, step: 0.01 },
        sweep: { value: 0.63, min: 0, max: 1, step: 0.01 },
        offsetX: { value: 0.42, min: 0, max: 1, step: 0.01 },
        offsetY: { value: 0.02, min: -1, max: 1, step: 0.01 },
        splayDeg: { value: 0, min: -45, max: 45, step: 1 },
    }, { collapsed: true })
